    ...
    function next () {
        ...
        if (toProcess) {
            ...
            promise.then(function (response) {
                ...
                if(response !== undefined && repl.xclip && !repl.multiPrompt) {
                    var cmd = "echo -n '" + JSON.stringify(response).split("'").join("'\"'\"'") + "' | xclip -i -selection c";
                    cp.exec(cmd);
                }
...
REPL.prototype.xclip = function (enable) {
    this.xclipEnabled = !!enable;
};
