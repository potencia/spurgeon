...
function groupResults (groups, verse) {
    var last = groups.pop();
    if (!last
    || last.version !== verse.version
    || last.book !== verse.book
    || last.chapter !== verse.chapter
    || (last.endVerse + 1) !== verse.verse) {
        if (last) {
            groups.push(last);
        }
        last = {
...
function buildNormalized (passage) {
    var afterColon = [passage.startVerse];
    if (passage.startVerse !== passage.endVerse) {
        afterColon.push(passage.endVerse);
    }
    afterColon = afterColon.join('-');
    return {
        reference : [passage.book, [passage.chapter, afterColon].join(':')].join(' '),
