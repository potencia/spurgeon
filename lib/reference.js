...
function storeChapterMetadata (chapter) {
    var newKey = [];
    Array.prototype.push.apply(newKey, this.key);
    newKey.push(chapter);
    liveMetadata[newKey.join(':')] = {
        verseCount : this.chapters[chapter].verseCount
    };
}

function storeBookMetadata (book) {
    var newKey = [];
    Array.prototype.push.apply(newKey, this.key);
    newKey.push(book);
    Object.keys(this.books[book]).forEach(storeChapterMetadata, {key : newKey, chapters : this.books[book]});
}

function storeVersionMetadata (version) {
    Object.keys(this.versions[version]).forEach(storeBookMetadata, {key : [version], books : this.versions[version]});
}

function validVerse (verse) {
    var error, tmp, missingProperties = [], emptyStrings = [], invalidTypes = {}, bookNoSpaces, bookId, unrecognizedBook = false;
    ...
    [ { property : 'book',
        ...
        if (required.property === 'book') {
            bookNoSpaces = verse[required.property].replace(/\s/g, '');
            bookId = bookAbbrMap[bookNoSpaces.toUpperCase()];
            if (bookId === undefined) {
...
function groupPassages (groups, verse) {
    var key, last = groups.pop();
    if (!last) {
        last = startNewGroup(verse);
    } else {
        if (last.version !== verse.version || last.book !== verse.book) {
            groups.push(last);
            last = startNewGroup(verse);
        } else {
            if (last.endChapter !== verse.chapter || (last.endVerse + 1) !== verse.verse) {
                key = [last.version, last.book, last.endChapter].join(':');
                if (liveMetadata[key] && (last.endChapter + 1) === verse.chapter && verse.verse === 1 && last.endVerse === liveMetadata[key].verseCount) {
                    last.endChapter = verse.chapter;
...
function passageComparator (a, b) {
    var comp = a.bookId - b.bookId;
    if (comp !== 0) {
        return comp;
    }

    comp = a.startChapter - b.startChapter;
    if (comp !== 0) {
        return comp;
    }

    comp = a.startVerse - b.startVerse;
    if (comp !== 0) {
        return comp;
    }

    comp = b.verses.length - a.verses.length;
    if (comp !== 0) {
        return comp;
    }
...
function innerVerse (verse) {
    return {
        book : fullBook[verse.bookId],
...
function buildNormalized (passage) {
    var numbers, bookName = fullBook[passage.bookId];
    if (oneChapter.indexOf(bookName) === -1) {
        numbers = [[passage.startChapter, passage.startVerse].join(':')];
        if (passage.startChapter !== passage.endChapter || passage.startVerse !== passage.endVerse) {
            if (passage.startChapter === passage.endChapter) {
                numbers.push(passage.endVerse);
            } else {
                numbers.push([passage.endChapter, passage.endVerse].join(':'));
            }
        }
    } else {
        numbers = [passage.startVerse];
        if (passage.startVerse !== passage.endVerse) {
            numbers.push(passage.endVerse);
        }
    }
    return {
        reference : [bookName, numbers.join('-')].join(' '),
        version : passage.version,
        verses : passage.verses.map(innerVerse)
    };
}

Reference.normalize = function (verses, metadata) {
    if (metadata) {
        Object.keys(metadata).forEach(storeVersionMetadata, {versions : metadata});
    }
    ...
    .sort(verseComparator)
    .reduce(groupPassages, [])
